...
<!-- use ratio of CPUEs between fall and spring, and assume same capture probability in both periods to estimate abundance in the spring. Recognize that capture probability is probably smaller in the spring, so our estimate is a conservative one (the truth is probably higher). Then re-do the bioenergetics model to look at how many smolts are consumed in the spring. -->

To estimate the abundance of Northern Pikeminnow in the spring, we used the ratio of total CPUE in a fall sampling event (total Northern Pikeminnow caught / effort) to the total CPUE in the spring, and multiply that ratio by the abundance of Northern Pikeminnow in the fall. This assumes the same capture probability in the fall as in the spring. If that does differ, we believe the capture probability to be lower in the spring due to higher flows, which would lead to our spring abundance estimate to be biased low. Therefore, we consider this a conservative estimate of the number of Northern Pikeminnow present in the Deadwater area during the spring Chinook smolt out migration.
