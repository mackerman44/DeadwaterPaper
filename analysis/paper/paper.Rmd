...
where $M$, $n$, and $m$ are indexed by each sample, $i$. Here, sample $i$ is defined as each day. The Schnabel estimator does not have an associated standard error; however, the 95% confidence intervals were calculated. The Schnabel estimator is essentially a weighted average of a series of Lincoln-Peterson estimators (with a Champman modification). Lastly, we explored one additional multiple-census estimator, the Schumacher-Eschmeyer, which is based on minimizing the weighted sum of squares between the proportion of marked individuals in the sample and the unknown proportion of marked individuals in the population. The Schumacher-Eschmeyer estimator is as follows:
...
We estimated an individual, average-size (`r avg_fall_NPM` mm) Northern Pikeminnow to consume `r median(fall_g_summ$cumm_g)` g (range: `r min(fall_g_summ$cumm_g)` - `r max(fall_g_summ$cumm_g)` g) of fish during the DSR emigration to maintain their body size, depending on the proportion of diet consisting of fish versus invertebrates (Figure \@ref(fig:bio-p)). Similarly, we estimated an average-size (`r avg_spring_NPM` mm) Northern Pikeminnow to consume `r median(spring_g_summ$cumm_g)` g (range: `r min(spring_g_summ$cumm_g)` - `r max(spring_g_summ$cumm_g)` g) of fish during the NRR emigration (Figure \@ref(fig:bio-p)). Given a mean Northern Pikeminnow population size of `r prettyNum(unique(adult_impact$N[adult_impact$season == "Fall"]), big.mark = ",")` from fall estimates and `r prettyNum(round(unique(adult_impact$N[adult_impact$season == "Spring"]), 0), big.mark = ",")` for spring 2021 and using the median value from diet scenarios (i.e., 50% of their diet consists of fish), we'd estimate that the Northern Pikeminnow population can consume `r prettyNum(median(fall_g_summ$cumm_g) * unique(adult_impact$N[adult_impact$season == "Fall"]), big.mark = ",")` g (range: `r prettyNum(min(fall_g_summ$cumm_g) * unique(adult_impact$N[adult_impact$season == "Fall"]), big.mark = ",")` - `r prettyNum(max(fall_g_summ$cumm_g) * unique(adult_impact$N[adult_impact$season == "Fall"]), big.mark = ",")` g) fish during the DSR emigration and an additional `r prettyNum(round(median(spring_g_summ$cumm_g) * unique(adult_impact$N[adult_impact$season == "Spring"]), 0), big.mark = ",")` g (range: `r prettyNum(round(min(spring_g_summ$cumm_g) * unique(adult_impact$N[adult_impact$season == "Spring"]), 0), big.mark = ",")` - `r prettyNum(round(max(spring_g_summ$cumm_g) * unique(adult_impact$N[adult_impact$season == "Spring"]), 0), big.mark = ",")` g) during the NRR emigration in Deadwater Slough. In total, the Northern Pikeminnow population can consume `r prettyNum(median(fall_g_summ$cumm_g) * unique(adult_impact$N[adult_impact$season == "Fall"]) + median(spring_g_summ$cumm_g) * unique(adult_impact$N[adult_impact$season == "Spring"]), big.mark = ",")` g of fish during the DSR and NRR emigrations to maintain their population size if we assume 50% of their diet consists of fish.
